...
- **Phase 1 of this PR was rewritten in response to a 3-way code review.** The original throttle design was a peek-only `shouldDelay` API with single-entry-eviction math. Grok (`grok-4.20-beta-0309-reasoning`) flagged two HIGH findings in `/coderev` + `/6step` analysis: (1) multi-entry eviction under-delays when ≥2 large entries remain in-window; (2) TOCTOU race between `shouldDelay` and `record` across the `await sleep()` + `await generateContent()` boundary. Both fixed before the module ever integrated with `ask`/`code`. GPT code review timed out with a 403 from the ChatGPT Codex backend (upstream auth issue, not a review signal). Full `/6step` analysis retained in the session history. The updated T22 scope in `docs/FOLLOW-UP-PRS.md` reflects what actually shipped, including the `lastObservedNowMs` clamp and the API shape.
