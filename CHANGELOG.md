...
- **`AbortSignal` threading through preflight (T17 follow-up).** The user's `timeoutMs` `AbortController` (v1.6.0) is now wired into `countForPreflight` and onward into `client.models.countTokens` via the SDK's `CountTokensConfig.abortSignal`. Pre-v1.10.0 fix the preflight phase ignored the timeout — a hung `countTokens` request could outlast the user's stated wall-clock budget. On cancellation the SDK throws `AbortError` and the preflight falls through to the `bytes/3` graceful-degradation path (same as 429s and network errors).
- **Defensive payload-size cap on tier-2.** When the assembled `contents` payload would exceed 32 MB, the tier-2 path skips the API and falls back to the `bytes/3` heuristic. Today's defaults (`maxFilesPerWorkspace: 2_000`, `maxFileSizeBytes: 1_000_000`) plus the heuristic-tier gate keep typical workspaces well below this cap, but the bound future-proofs against config bumps and bounds the `'a'.repeat(file.size)` placeholder memory footprint on degenerate workspaces (where files become unreadable between scan and preflight).
