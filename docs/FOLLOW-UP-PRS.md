...
- Unit tests in `test/unit/throttle.test.ts` with fake timers — 33 tests covering: disabled path, single-entry math, multi-entry eviction with post-delay invariant assertion, TOCTOU race (second concurrent caller sees first caller's provisional), release-with-actual-{less,greater}-than-estimate, cancel, idempotency of release/cancel on unknown IDs (including release-after-cancel of same id), unique release IDs, explicit-nowMs `release` prune path, `shouldDelay` non-inflation invariant, oversize-estimate lockout (deliberate 60s block after a single over-limit call), per-model isolation, retry hints, non-monotonic clock, non-finite `nowMs` poisoning guard, input validation.
