...
| ~~C~~ | ~~**v1.4.0**~~ ✅ SHIPPED | ~~#22~~ | ~~T24 — Model taxonomy with allowlist-first category system. Tools bind to required category; resolver refuses to dispatch outside it. Closes the `nano-banana-doing-code-review` class of bug at its root (flips v1.2.0's blocklist defence).~~ | ~~~6 h~~ |
