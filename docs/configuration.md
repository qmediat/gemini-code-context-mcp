...
| `GEMINI_CODE_CONTEXT_CACHING_MODE` *(v1.14.0+)* | `implicit` | Operator-level default for the `cachingMode` field on `ask` / `code`. Accepts `'explicit'` or `'implicit'` (case-insensitive). **`'implicit'` (default since v1.14.0)** skips `caches.create` entirely; file content is sent inline and Gemini 2.5+/3 Pro applies its automatic implicit caching to repeated workspace prefixes — eliminates the 60–180 s rebuild wait on the review→edit→review path. **`'explicit'`** builds a Gemini Context Cache for guaranteed ~75 % discount on cached input tokens; pays the rebuild cost on every file change. Set `'explicit'` if you need predictable per-call billing more than warm-path latency. Per-call `input.cachingMode` always wins. Invalid values (mistypes, unrecognised options) emit a stderr warning AND fall back to `'implicit'` so the override doesn't silently no-op. Hit-rate observability via `status.structuredContent.caching.implicitHitRate`. |
...
- `ask({ cachingMode: "explicit" })` / `code({ cachingMode: "explicit" })` *(v1.13.0+)* — opt INTO an explicit Gemini Context Cache instead of relying on Gemini 2.5+/3 Pro's automatic implicit caching (the v1.14.0 default). **`"implicit"`** (default since v1.14.0) skips `caches.create` entirely; file content is sent inline every call and Gemini's automatic implicit cache discounts the prefix when it matches across calls. Optimised for review→edit→review workflows — no rebuild wait when files change between queries. **`"explicit"`** builds a `caches.create` cache for guaranteed ~75 % discount on cached input tokens but pays the 60–180 s rebuild cost on every file change. Set `"explicit"` if you need predictable per-call billing more than warm-path latency. The trade-off documented in [ai.google.dev/gemini-api/docs/caching](https://ai.google.dev/gemini-api/docs/caching): implicit caching has no cost-saving guarantee. Hit rate observable via `status.structuredContent.caching.implicitHitRate`; the `status` tool warns inline if your implicit hit rate falls below 50 %. Operator-level override: env `GEMINI_CODE_CONTEXT_CACHING_MODE`.
